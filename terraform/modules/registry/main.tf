...
resource "null_resource" "docker_build_push" {
  ...
  triggers = {
    src_hash = sha256(join("", [
      filesha256("${path.module}/../../../api/Dockerfile"),
      filesha256("${path.module}/../../../api/requirements.txt"),
    ]))
  }
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      docker build --platform linux/amd64 --provenance=false -t ${local.image} ${path.module}/../../../api
