...
async def _ensure_report_type_exists(db: Any, report_type: str) -> None:
    cursor = await db.execute(
        "SELECT id FROM report_types WHERE id=? AND is_active LIMIT 1", (report_type,)
    )
...
async def get_nearby_reports(
    ...
    async with get_db() as db:
        try:
            cursor = await db.execute(
                "SELECT * FROM community_reports WHERE lat BETWEEN ? AND ? AND lng BETWEEN ? AND ? AND is_active",
                (min_lat, max_lat, min_lng, max_lng),
            )
            rows = await cursor.fetchall()
            return {"reports": [report_row_to_dict(r) for r in rows]}
...
async def get_my_reports(request: Request):
    ...
    async with get_db() as db:
        try:
            ...
            rows = await cursor.fetchall()
            report_list = [report_row_to_dict(r) for r in rows]
...
async def get_report_types():
    async with get_db() as db:
        try:
            cursor = await db.execute("SELECT * FROM report_types WHERE is_active ORDER BY sort_order")
            rows = await cursor.fetchall()
...
async def get_report(report_id: str, request: Request):
    ...
    async with get_db() as db:
        try:
            ...
            return _with_viewer_vote(report_row_to_dict(row), viewer_votes.get(report_id, 0))
    ...
    async with get_db() as db:
        try:
            ...
            await insert_community_report(
                ...
                report_id=report_id,
                created_by=user_id,
                anon_fingerprint=getattr(req, "anon_fingerprint", None),
                report_type=req.report_type,
                title=req.title,
                description=req.description,
                lat=req.lat,
                lng=req.lng,
                address_hint=getattr(req, "address_hint", None),
                created_at=now,
            ...
            cursor = await db.execute("SELECT * FROM community_reports WHERE id=?", (report_id,))
            row = await cursor.fetchone()
            return {"report": _with_viewer_vote(report_row_to_dict(row), 0)}
        except HTTPException:
            raise
        except Exception as e:
...
async def confirm_report(report_id: str, req: Any, request: Request):
    ...
    async with get_db() as db:
        ...
        try:
            ...
            cursor = await db.execute("SELECT * FROM community_reports WHERE id=?", (report_id,))
            row = await cursor.fetchone()
            return {"status": "ok", "vote": req.vote, "report": _with_viewer_vote(report_row_to_dict(row), req.vote)}
        except Exception as e:
