...
ADMIN_UIDS = {"5duJR57R28cOaVgKQBv9EyAClXp2"}
...
async def sync_profile(body: SyncProfileBody, request: Request):
    ...
    async with get_db() as db:
        try:
            ...
            if firebase_uid in ADMIN_UIDS:
                await db.execute(
                    "UPDATE profiles SET role = 'admin', updated_at = CURRENT_TIMESTAMP WHERE firebase_uid = ?",
                    (firebase_uid,),
                )
            elif firebase_uid in DEV_BUSINESS_PROFILES:
